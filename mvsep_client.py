...
class MVSEPClient:
    ...
    def create_separation(self, file_path: Optional[str] = None, url: Optional[str] = None,
        ...
        if file_path:
            ...
            # Use context manager to ensure file descriptor is closed after request
            ...
            response = self._make_request("POST", "separation/create", data=data, files=None)
            json_response = response.json()
            self._log_debug(f"Create separation response: {json_response}")
            return json_response
        else:
    ...
    def create_quality_entry(self, zip_path: str, algo_name: str, main_text: str,
        ...
        # Use context manager so the zip file descriptor is closed
    ...
    def submit_file(self, file_path: str, sep_type: int = 11,
                    add_opt1: Optional[Union[str, int]] = None,
                    add_opt2: Optional[Union[str, int]] = None,
                    add_opt3: Optional[Union[str, int]] = None,
                    output_format: int = 1) -> str:
        """Submit a local file and return the task hash (raises on failure)."""
        resp = self.create_separation(file_path=file_path, sep_type=sep_type,
                                      add_opt1=add_opt1, add_opt2=add_opt2, add_opt3=add_opt3,
                                      output_format=output_format)
        if not resp.get("success"):
            raise Exception(f"Create separation failed: {resp}")
        return resp["data"]["hash"]

    def wait_for_done(self, task_hash: str, poll_interval: Optional[int] = None, timeout: int = 3600) -> Dict:
        """Poll the server until the task is done. Returns the final status response."""
        poll_interval = poll_interval or self.retry_interval
        start = time.time()
        while True:
            status_resp = self.get_separation_status(task_hash)
            status = status_resp.get("status")
            ...
            if status == "done":
                return status_resp
            if status in ("failed", "error"):
                raise Exception(f"Separation failed: {status_resp}")
            if time.time() - start > timeout:
                raise TimeoutError("Waiting for separation timed out")
            time.sleep(poll_interval)

    def download_result(self, status_resp: Dict, output_dir: str) -> None:
        """Download all files from a successful status response into `output_dir`."""
        files = status_resp.get("data", {}).get("files", [])
        os.makedirs(output_dir, exist_ok=True)
        for file_info in files:
            filename = file_info.get("download") or file_info.get("name") or f"unknown_{time.time()}"
            out_path = os.path.join(output_dir, filename)
            url = file_info.get("url") or file_info.get("link")
            if not url:
                self._log_debug(f"No URL found for file entry: {file_info}")
                continue
            self._log_debug(f"Downloading result file {filename} to {out_path}")
            self.download_track(url, out_path)
